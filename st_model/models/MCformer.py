...
from st_model.layers.MCformer_backbone import MCformer_backbone
from st_model.layers.PatchTST_layers import series_decomp
...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len:Optional[int]=1024, d_k:Optional[int]=None, d_v:Optional[int]=None, norm:str='BatchNorm', attn_dropout:float=0., 
        ...
        stack_len = configs.stack_len

        # model
        self.decomposition = decomposition
        if self.decomposition:
            self.decomp_module = series_decomp(kernel_size)
            self.model_trend = MCformer_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                                 ...
                                                 subtract_last=subtract_last, verbose=verbose, stack_len=stack_len,
                                                 **kwargs)
            self.model_res = MCformer_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                               patch_len=patch_len, stride=stride, max_seq_len=max_seq_len,
                                               n_layers=n_layers, d_model=d_model, n_heads=n_heads, d_k=d_k, d_v=d_v,
                                               d_ff=d_ff, norm=norm, attn_dropout=attn_dropout, dropout=dropout,
                                               act=act, key_padding_mask=key_padding_mask, padding_var=padding_var,
                                               ...
                                               revin=revin, affine=affine, subtract_last=subtract_last, verbose=verbose,
                                               stack_len=stack_len, **kwargs)
        else:
            self.model = MCformer_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                           patch_len=patch_len, stride=stride, max_seq_len=max_seq_len,
                                           n_layers=n_layers, d_model=d_model, n_heads=n_heads, d_k=d_k, d_v=d_v,
